...
export function SettingsForm({ role }: { role: string }) {
  const [provider, setProvider] = useState({ apiKey: '', baseUrl: '' })
  ...
  async function saveProvider(e: React.FormEvent) {
    ...
    if (res.ok) { setSaved('provider'); setTimeout(() => setSaved(null), 3000) }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="font-semibold text-slate-800">Credenciales de proveedor</h2>
        </div>
        <p className="text-sm text-slate-500 mb-4">
          {role === 'admin'
            ? 'Credenciales del proveedor de mensajería. Tus clientes nunca las verán.'
            : 'Credenciales para el servicio de mensajería. Contáctanos si necesitas las tuyas.'}
        </p>
        <form onSubmit={saveProvider} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex">
              ...
              <input
                ...
                placeholder={role === 'admin' ? 'xxxxxx.api.infobip.com' : 'Tu URL de acceso'}
          ...
          <div className="flex items-center gap-3">
            ...
            {saved === 'provider' && <span className="text-sm text-emerald-600">✓ Guardado</span>}
          </div>
        </form>
      </div>
